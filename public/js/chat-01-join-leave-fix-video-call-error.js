...
class WebRTCCall {
    ...
    async _getUserMediaWithFallback() {
        // First check if we've already determined camera is unavailable
        if (this.cameraAccessDenied) {
            throw new Error('Camera access previously denied');
        }
        ...
        try {
            ...
            if (firstError.name === 'NotReadableError') {
                ...
                throw new Error('Camera not readable');
            }

            // For OverconstrainedError, try removing constraints
            try {
                ...
                console.warn('Simple constraints failed:', secondError);
  ...
  _handleFailure(error) {
    ...
    if (error.message.includes('Camera access denied')) {
        userMessage = 'Please enable camera permissions in your browser settings';
    } else if (error.message.includes('Camera not readable')) {
        userMessage = 'Camera is not available. Please check your device';
    } else if (error.message.includes('audio only')) {
        userMessage = 'Continuing with audio only';
    }

    this._showUserAlert(userMessage);
...
_showUserAlert(message) {
    // Implement your UI alert system here
    const alertBox = document.createElement('div');
    alertBox.className = 'media-alert';
    alertBox.textContent = message;
    document.getElementById('callContainer').appendChild(alertBox);
